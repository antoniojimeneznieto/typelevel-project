...
import info.antoniojimenez.jobsboard.fixtures.JobFixture
import org.typelevel.log4cats.Logger
import org.typelevel.log4cats.slf4j.Slf4jLogger
import info.antoniojimenez.jobsboard.domain.job.JobFilter
  ...
  "Jobs 'algebra'" - {
    ...
    "should create a new job" in {
      transactor.use { xa =>
        val program = for {
          jobs     <- LiveJobs[IO](xa)
          jobId    <- jobs.create("antonio@antoniojimenez.com", RockTheJvmNewJob)
    ...
    "should create an updated job if it exists" in {
      transactor.use { xa =>
        val program = for {
          jobs            <- LiveJobs[IO](xa)
    ...
    "should filter jobs by tags" in {
      transactor.use { xa =>
        val program = for {
          jobs         <- LiveJobs[IO](xa)
          filteredJobs <- jobs.all(JobFilter(tags = List("scala", "cats", "doobie")), Pagination.default)
        } yield filteredJobs
